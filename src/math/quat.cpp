...
vec3 operator*(const quat& q, const vec3& v) {
	return vec3(q.x, q.y, q.z) * 2.0f * dot(vec3(q.x, q.y, q.z), v) +
		v * (q.w * q.w - dot(vec3(q.x, q.y, q.z), vec3(q.x, q.y, q.z))) +
		cross(vec3(q.x, q.y, q.z), v) * 2.0f * q.w;
}
...
quat operator^(const quat& q, float f) {
	...
	vec3 axis = normalized(vec3(q.x, q.y, q.z));
...
quat mat4ToQuat(const mat4& m) {
	vec3 up = normalized(vec3(m.yx, m.yy, m.yz));
	vec3 forward = normalized(vec3(m.zx, m.zy, m.zz));
